...
empRegister.controller('EmployeeController', ['$http','$scope', function($http, $scope) {
...
/* Inserting employee education details */
	...
	$scope.saveeducation = function(UserData) {
		UserData.empid = Andromeda.getSessionValue("employeeid");
        console.log(UserData.empid);
		$http.post('/erp/employee/data',UserData).then(
				function(response) {    
					$scope.data = response.data;      
					if ($scope.data.successful) {
			    		$("#com").show(); 
...
/* Getting table data in education tab in employee profile */	
	$scope.getById6 = function(empid){
		/*alert(empid);*/   
		$http.post('/erp/employee/getById6', empid).then(  
				function(response) {   
					$scope.data = response.data;
					if ($scope.data.successful) {
						$scope.data={object:$scope.data.responseObject};
